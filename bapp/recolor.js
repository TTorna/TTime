...
async function recolor() {
  try {
    ...
    image.scan(0, 0, image.bitmap.width, image.bitmap.height, function(x, y, idx) {
      ...
      if (alpha > 0) {
        // Only recolor if the pixel has color (don't tint fully white or fully black unless it's a solid icon)
        // Typically for an icon we just tint everything that's not transparent.
        // If it's a black/grey icon with alpha blending, setting the RGB directly works perfectly to colorize it.
        this.bitmap.data[idx] = 103;     // Red
        this.bitmap.data[idx + 1] = 12;  // Green
        this.bitmap.data[idx + 2] = 178; // Blue
        // We leave the alpha channel as it is to preserve the smooth edges (anti-aliasing)
      }
    });
